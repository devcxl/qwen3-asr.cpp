...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<asr::transcribe_result>(m, "TranscribeResult",
      ...
      .def("__repr__", [](const asr::transcribe_result &r) {
        if (r.success) {
          return "<TranscribeResult success text=\"" + r.text.substr(0, 80) +
                 "\">";
        }
        return "<TranscribeResult success=false error=\"" + r.error_msg + "\">";
      });

  // ---- Qwen3ASR (RAII — destructor frees ggml resources) ---------------
  py::class_<asr::Qwen3ASR>(
      m, "Qwen3ASR",
      ...
      .def(py::init<>())
      .def("load_model", &asr::Qwen3ASR::load_model, py::arg("model_path"),
           "Load ASR model from a GGUF file. Returns True on success.")
      .def("transcribe",
           py::overload_cast<const std::string &,
                             const asr::transcribe_params &>(
               &asr::Qwen3ASR::transcribe),
           py::arg("audio_path"), py::arg("params") = asr::transcribe_params(),
      ...
      .def(
          ...
          [](asr::Qwen3ASR &self,
            ...
            return self.transcribe(static_cast<const float *>(buf.ptr),
                                   static_cast<int>(buf.size), params);
          },
          py::arg("samples"), py::arg("params") = asr::transcribe_params(),
      ...
      .def("__exit__", [](asr::Qwen3ASR &self, py::object, py::object,
                          py::object) { /* RAII */ });

  // ---- Free functions ---------------------------------------------------
  m.def("load_audio_file", &py_load_audio_file, py::arg("path"),
