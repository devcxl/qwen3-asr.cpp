...
std::string asr_server::json_pair(const std::string &key, const std::string &val) {
...
std::string asr_server::json_pair(const std::string &key, double val, int precision) {
...
asr_server::asr_server(const asr_server_params &params)
    : params_(params)
{
}
...
void asr_server::handle_health(const httplib::Request & /*req*/, httplib::Response &res) {
...
void asr_server::handle_models(const httplib::Request & /*req*/, httplib::Response &res) {
    std::string model_entry = json_object({
        json_pair("id", "qwen3-asr"),
        json_pair("object", "model"),
        json_pair("created", static_cast<long>(std::time(nullptr))),
        json_pair("owned_by", "user")
    });

    std::string body = json_object({
        json_pair("object", "list"),
        "\"data\": " + json_array({model_entry})
    });
    res.set_content(body, "application/json");
}

void asr_server::handle_transcriptions(const httplib::Request &req, httplib::Response &res) {
    if (!ready_.load()) {
        std::string body = json_object({
    ...
    if (!parse_wav_from_memory(raw_bytes, sample_rate, channels, audio_data)) {
        std::string body = json_object({
            json_pair("error", "Bad Request"),
            json_pair("message", "Failed to parse WAV audio file. Only PCM WAV is supported.")
        });
        res.status = 400;
        res.set_content(body, "application/json");
        return;
    ...
    if (slot_id < 0) {
        std::string body = json_object({
            ...
            json_pair("message", "All inference slots are busy. Please try again later.")
        });
        res.status = 429;
        res.set_content(body, "application/json");
        return;
    ...
    try {
        ...
        auto result = model_->transcribe(audio_data.data(), (int)audio_data.size(), tp);
        if (result.success) {
    ...
    if (response_format == "text") {
        ...
        ss << "1\n"
           ...
           << std::setw(2) << std::setfill('0') << s << ",000 --> "
           << std::setw(2) << std::setfill('0') << h << ":"
           << std::setw(2) << std::setfill('0') << m << ":"
           << std::setw(2) << std::setfill('0') << s << ",000\n"
           << text << "\n\n";
        ...
        double dur = (double)audio_data.size() / (double)sample_rate;
        std::string body = json_object({
            json_pair("task", "transcribe"),
            json_pair("language", "chinese"),
            json_pair("duration", dur),
            json_pair("text", text),
            "\"segments\": []"
        });
        res.set_content(body, "application/json");
...
bool asr_server::parse_wav_from_memory(const std::vector<uint8_t> &data,
    ...
    if (audio_format != 1 && audio_format != 3) return false; // PCM or IEEE float
    ...
    if (channels == 0 || sample_rate == 0 || bits_per_sample == 0) return false;
    ...
    if (data_size == 0 || data_start + data_size > data.size()) return false;
...
std::vector<float> asr_server::convert_to_mono_16k(const std::vector<float> &samples,
                                                     int sample_rate, int channels) {
    // Mix to mono
    ...
    if (sample_rate == 16000) return mono;
...
int main(int argc, char **argv) {
    ...
    if (params.n_parallel < 1) params.n_parallel = 1;
    if (params.n_parallel > 32) params.n_parallel = 32;
