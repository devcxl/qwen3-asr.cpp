...
std::vector<int32_t> find_audio_positions(const int32_t *input_ids,
                                          int32_t n_tokens,
...
void embed_tokens(const int32_t *input_ids, int32_t n_tokens,
                  const float *token_embd, int32_t vocab_size,
                  int32_t hidden_size, float *output) {
...
bool inject_audio_embeddings(float *token_embeddings, int32_t n_tokens,
                             int32_t hidden_size, const float *audio_features,
...
injection_result inject_audio(const int32_t *input_ids, int32_t n_tokens,
  ...
  std::vector<int32_t> audio_positions =
      find_audio_positions(input_ids, n_tokens, ctx.tokens.audio_pad_token_id);

  if (audio_features && n_audio_frames > 0) {
    if (static_cast<int32_t>(audio_positions.size()) != n_audio_frames) {
      result.error_msg = "Mismatch: " + std::to_string(audio_positions.size()) +
                         " audio_pad tokens but " +
  ...
  embed_tokens(input_ids, n_tokens, ctx.token_embd, ctx.vocab_size,
               ctx.hidden_size, result.embeddings.data());

  if (audio_features && n_audio_frames > 0 && !audio_positions.empty()) {
    if (!inject_audio_embeddings(result.embeddings.data(), n_tokens,
                                 ctx.hidden_size, audio_features,
                                 n_audio_frames, audio_positions)) {
...
bool validate_audio_injection(const int32_t *input_ids, int32_t n_tokens,
                              int32_t n_audio_frames,
                              int32_t audio_pad_token_id,
                              std::string &error_msg) {

  int32_t pad_count =
      count_audio_pad_tokens(input_ids, n_tokens, audio_pad_token_id);
...
int32_t find_audio_start_position(const int32_t *input_ids, int32_t n_tokens,
                                  int32_t audio_pad_token_id) {
...
int32_t count_audio_pad_tokens(const int32_t *input_ids, int32_t n_tokens,
                               int32_t audio_pad_token_id) {
