...
bool load_wav(const std::string &path, std::vector<float> &samples,
              int &sample_rate) {
...
bool load_mel_filters_npy(const std::string &path, MelFilters &filters) {
  ...
  if (shape.size() != 2) {
    fprintf(stderr,
            "Error: Expected 2D array for mel filters, got %zu dimensions\n",
            shape.size());
    return false;
  }
  ...
  if (dtype == "<f8" || dtype == "float64") {
    ...
    for (size_t i = 0; i < n_fft; i++) {
      for (size_t j = 0; j < n_mel; j++) {
        filters.data[j * n_fft + i] =
            static_cast<float>(raw_data[i * n_mel + j]);
      }
    }
  } else if (dtype == "<f4" || dtype == "float32") {
    std::vector<float> raw_float(n_fft * n_mel);
    file.read(reinterpret_cast<char *>(raw_float.data()),
              raw_float.size() * sizeof(float));
    // Transpose from (n_fft, n_mel) to (n_mel, n_fft)
    for (size_t i = 0; i < n_fft; i++) {
      for (size_t j = 0; j < n_mel; j++) {
...
void generate_mel_filters(MelFilters &filters, int n_mels, int n_fft,
                          int sample_rate) {
...
bool log_mel_spectrogram(const float *samples, int n_samples,
  ...
  // Center padding: n_fft//2 on each side (matches HuggingFace/librosa
  // center=True)
  ...
  int total_frames =
      (static_cast<int>(samples_padded.size()) - frame_size) / frame_step + 1;
  ...
  for (int i = 0; i < compute_frames; i++) {
    ...
    vDSP_vmul(hann_f, 1, &samples_padded[offset], 1, windowed.data(), 1,
              frame_size);

    cblas_sgemv(CblasRowMajor, CblasNoTrans, n_fft, frame_size, 1.0f,
                W_cos.data(), frame_size, windowed.data(), 1, 0.0f,
                dft_re.data(), 1);
    cblas_sgemv(CblasRowMajor, CblasNoTrans, n_fft, frame_size, -1.0f,
                W_sin.data(), frame_size, windowed.data(), 1, 0.0f,
                dft_im.data(), 1);
    ...
    for (int j = 0; j < mel.n_mel; j++) {
      ...
      temp_data[j * compute_frames + i] =
          log10(std::max(static_cast<double>(dot), 1e-10));
    }
  }
...
bool save_mel_npy(const std::string &path, const MelSpectrogram &mel) {
  ...
  header +=
      std::to_string(mel.n_mel) + ", " + std::to_string(mel.n_len) + "), }";
...
float compare_mel(const MelSpectrogram &a, const MelSpectrogram &b) {
  if (a.n_mel != b.n_mel || a.n_len != b.n_len) {
    fprintf(stderr,
            "Error: Mel spectrogram dimensions don't match: "
            "(%d, %d) vs (%d, %d)\n",
