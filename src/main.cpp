...
static void print_usage(const char *prog) {
  ...
  fprintf(stderr, "  -m, --model <path>     Path to GGUF model (default: "
                  "models/qwen3-asr-0.6b-f16.gguf)\n");
  fprintf(stderr, "  -f, --audio <path>     Path to audio file (WAV, 16kHz "
                  "mono) [required]\n");
  fprintf(stderr,
          "  -o, --output <path>    Output file path (default: stdout)\n");
  fprintf(stderr, "  -l, --language <code>  Language code (optional, e.g. "
                  "'korean' for Korean word splitting)\n");
  ...
  fprintf(stderr, "  --profile              Print detailed timing profile "
                  "(requires QWEN3_ASR_TIMING build)\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "Forced Alignment:\n");
  fprintf(stderr, "  --align                Enable forced alignment mode\n");
  fprintf(stderr,
          "  --text <text>          Reference transcript for alignment\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "Transcribe + Align:\n");
  fprintf(stderr, "  -a, --transcribe-align Run ASR then forced alignment\n");
  fprintf(stderr, "  --aligner-model <path> Path to forced aligner GGUF model "
                  "(required with --transcribe-align)\n");
  fprintf(stderr, "\n");
  ...
  fprintf(stderr, "  Forced Alignment:\n");
  fprintf(stderr,
          "    %s -m models/qwen3-forced-aligner-0.6b-f16.gguf -f sample.wav "
          "--align --text \"Hello world\"\n",
          prog);
  fprintf(stderr, "\n");
  fprintf(stderr, "  Transcribe + Align:\n");
  fprintf(stderr,
          "    %s -m models/qwen3-asr-0.6b-f16.gguf --aligner-model "
          "models/qwen3-forced-aligner-0.6b-f16.gguf -f sample.wav "
          "--transcribe-align\n",
          prog);
}
...
static bool parse_args(int argc, char **argv, cli_params &params) {
  ...
  if (params.align_mode && params.align_text.empty()) {
    fprintf(stderr,
            "Error: Reference text is required for alignment mode (--text)\n");
    return false;
  }

  if (params.align_mode && params.transcribe_align_mode) {
    fprintf(stderr,
            "Error: --align and --transcribe-align cannot be used together\n");
    return false;
  }
...
static std::string detect_language(const std::string &asr_text) {
  ...
  if (asr_text.size() < prefix.size() ||
      asr_text.compare(0, prefix.size(), prefix) != 0) {
    return "";
  }
...
static std::string
alignment_to_json(const qwen3_asr::alignment_result &result) {
  std::string json = "{\n  \"words\": [\n";
...
static int run_alignment(const cli_params &params) {
  ...
  if (params.language == "korean") {
    ...
    if (dict_path.empty()) {
      fprintf(stderr, "Warning: Korean dictionary not found. Falling back to "
  ...
  auto result =
      aligner.align(params.audio_path, params.align_text, params.language);
...
static int run_transcribe_and_align(const cli_params &params) {
  ...
  std::string align_lang =
      params.language.empty() ? detected_lang : params.language;
...
static void ggml_log_callback_quiet(enum ggml_log_level level, const char *text,
                                    void *user_data) {
  (void)user_data;
