...
bool GGUFLoader::parse_hparams(struct gguf_context *ctx,
...
bool GGUFLoader::create_tensors(struct gguf_context *ctx,
                                audio_encoder_model &model) {
  ...
  for (int64_t i = 0; i < n_tensors; ++i) {
    ...
    if (strstr(name, "encoder.conv1.weight")) {
      ...
      n_dims = 2;
               ...
               strstr(name, "attn_v.weight") ||
               strstr(name, "attn_out.weight")) {
...
bool GGUFLoader::load_tensor_data(const std::string &path,
  ...
  if (gpu_dev) {
    model.buffer = ggml_backend_dev_buffer_from_host_ptr(
