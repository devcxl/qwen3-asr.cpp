...
public:
  ...
  std::vector<int32_t>
  tokenize_with_timestamps(const std::string &text,
...
private:
  ...
  struct ggml_cgraph *build_decoder_graph(const int32_t *tokens,
                                          int32_t n_tokens,
  ...
  bool forward_decoder(const int32_t *tokens, int32_t n_tokens,
                       const float *audio_embd, int32_t n_audio,
                       int32_t audio_start_pos, std::vector<float> &output);
  ...
  std::vector<int32_t>
  extract_timestamp_classes(const std::vector<float> &logits,
                            const std::vector<int32_t> &tokens,
                            int32_t timestamp_token_id);

  // Build input sequence with audio placeholders
  std::vector<int32_t>
  build_input_tokens(const std::vector<int32_t> &text_tokens,
