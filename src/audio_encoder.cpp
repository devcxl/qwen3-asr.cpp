...
struct ggml_cgraph *AudioEncoder::build_graph_conv(int n_frames) {
  ...
  struct ggml_tensor *mel =
      ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_frames, n_mel);
  ggml_set_name(mel, "mel");
  ggml_set_input(mel);

  struct ggml_tensor *mel_4d =
      ggml_reshape_4d(ctx0, mel, n_frames, n_mel, 1, 1);

  struct ggml_tensor *cur =
      ggml_conv_2d(ctx0, model_.conv2d1_w, mel_4d, 2, 2, 1, 1, 1, 1);
  if (model_.conv2d1_b) {
    struct ggml_tensor *bias =
        ggml_reshape_4d(ctx0, model_.conv2d1_b, 1, 1, conv_ch, 1);
  ...
  if (model_.conv2d2_b) {
    struct ggml_tensor *bias =
        ggml_reshape_4d(ctx0, model_.conv2d2_b, 1, 1, conv_ch, 1);
    cur = ggml_add(ctx0, cur, bias);
  }
  cur = ggml_gelu(ctx0, cur);
  ...
  if (model_.conv2d3_b) {
    struct ggml_tensor *bias =
        ggml_reshape_4d(ctx0, model_.conv2d3_b, 1, 1, conv_ch, 1);
    cur = ggml_add(ctx0, cur, bias);
  }
  cur = ggml_gelu(ctx0, cur);
  ...
  for (int il = 0; il < n_layer; ++il) {
...

    {
      ...
      struct ggml_tensor *Q = ggml_permute(
          ctx0, ggml_reshape_3d(ctx0, Qcur, n_state_head, n_head, n_ctx), 0, 2,
          1, 3);

      struct ggml_tensor *K = ggml_permute(
          ctx0, ggml_reshape_3d(ctx0, Kcur, n_state_head, n_head, n_ctx), 0, 2,
          1, 3);
...
bool AudioEncoder::compute_graph(struct ggml_cgraph *graph) {
  ...
  if (ggml_backend_sched_graph_compute(state_.sched, graph) !=
      GGML_STATUS_SUCCESS) {
...
bool AudioEncoder::encode(const float *mel_data, int n_mel, int n_frames,
  ...
  if (n_mel != model_.hparams.n_mel_bins) {
    error_msg_ = "Mel bins mismatch: expected " +
                 std::to_string(model_.hparams.n_mel_bins) + ", got " +
                 std::to_string(n_mel);
    return false;
  }
  ...
  for (int chunk_idx = 0; chunk_idx < n_chunks; ++chunk_idx) {
    ...
    if (!ggml_backend_sched_alloc_graph(state_.sched, gf_conv)) {
      error_msg_ = "Failed to allocate conv graph for chunk " +
    ...
    ggml_backend_tensor_set(mel_tensor, chunk_mel.data(), 0,
                            n_mel * chunk_len * sizeof(float));

    if (ggml_backend_sched_graph_compute(state_.sched, gf_conv) !=
        GGML_STATUS_SUCCESS) {
      error_msg_ =
          "Failed to compute conv graph for chunk " + std::to_string(chunk_idx);
      ggml_backend_sched_reset(state_.sched);
      return false;
    }
    ...
    ggml_backend_tensor_get(embd_conv, chunk_output.data(), 0,
                            out_ctx * out_state * sizeof(float));
    ...
    all_conv_outputs.insert(all_conv_outputs.end(), chunk_output.begin(),
                            chunk_output.end());
  ...
  struct ggml_cgraph *gf_enc =
      ggml_new_graph_custom(enc_ctx, QWEN3_ASR_MAX_NODES, false);
  ...
  struct ggml_tensor *inpL =
      ggml_new_tensor_2d(enc_ctx, GGML_TYPE_F32, n_state, n_ctx);
  ...
  for (int il = 0; il < n_layer; ++il) {
...

    {
      ...
      struct ggml_tensor *Q = ggml_permute(
          enc_ctx, ggml_reshape_3d(enc_ctx, Qcur, n_state_head, n_head, n_ctx),
          0, 2, 1, 3);

      struct ggml_tensor *K = ggml_permute(
          enc_ctx, ggml_reshape_3d(enc_ctx, Kcur, n_state_head, n_head, n_ctx),
          0, 2, 1, 3);

      struct ggml_tensor *KQ = ggml_mul_mat(enc_ctx, K, Q);

      struct ggml_tensor *KQ_soft_max =
          ggml_soft_max_ext(enc_ctx, KQ, nullptr, KQscale, 0.0f);

      struct ggml_tensor *V = ggml_cont(
          enc_ctx, ggml_permute(enc_ctx,
                                ggml_reshape_3d(enc_ctx, Vcur, n_state_head,
                                                n_head, n_ctx),
                                1, 2, 0, 3));
  ...
  ggml_backend_tensor_set(enc_input, all_conv_outputs.data(), 0,
                          n_ctx * n_state * sizeof(float));

  {
    QWEN3_TIMER("audio_encoding.transformer");
    if (ggml_backend_sched_graph_compute(state_.sched, gf_enc) !=
        GGML_STATUS_SUCCESS) {
  ...
  ggml_backend_tensor_get(embd_enc, output.data(), 0,
...
bool AudioEncoder::encode_no_chunk(const float *mel_data, int n_mel,
                                   int n_frames, std::vector<float> &output) {
  if (!model_.ctx) {
    error_msg_ = "Model not loaded";
    return false;
  }
  ...
  ggml_backend_tensor_set(mel_tensor, transposed_mel.data(), 0,
                          n_mel * n_frames * sizeof(float));
  ...
  ggml_backend_tensor_get(embd_conv, conv_output.data(), 0,
                          out_ctx * out_state * sizeof(float));
  ...
  struct ggml_tensor *inpL =
      ggml_new_tensor_2d(enc_ctx, GGML_TYPE_F32, n_state, out_ctx);
  ggml_set_name(inpL, "enc_input");
  ggml_set_input(inpL);
  ...
  for (int il = 0; il < n_layer; ++il) {
...

    {
      ...
      struct ggml_tensor *Q = ggml_permute(
          enc_ctx,
          ggml_reshape_3d(enc_ctx, Qcur, n_state_head, n_head, out_ctx), 0, 2,
          1, 3);

      struct ggml_tensor *K = ggml_permute(
          enc_ctx,
          ggml_reshape_3d(enc_ctx, Kcur, n_state_head, n_head, out_ctx), 0, 2,
          1, 3);
  ...
  ggml_backend_tensor_set(enc_input, conv_output.data(), 0,
                          out_ctx * n_state * sizeof(float));
  ...
  ggml_backend_tensor_get(embd_enc, output.data(), 0,
                          final_ctx * final_state * sizeof(float));
...
bool AudioEncoder::encode_conv_only(const float *mel_data, int n_mel,
                                    int n_frames, std::vector<float> &output) {
  if (!model_.ctx) {
    error_msg_ = "Model not loaded";
    return false;
  ...
  ggml_backend_tensor_get(embd_conv, output.data(), 0,
                          out_ctx * out_state * sizeof(float));
