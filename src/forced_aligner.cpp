...
bool ForcedAligner::load_model(const std::string &model_path) {
  ...
  if (state_.backend_gpu) {
    ...
    backend_bufts.push_back(
        ggml_backend_get_default_buffer_type(state_.backend_gpu));
  }

  backends.push_back(state_.backend_cpu);
  ggml_backend_buffer_type_t cpu_buft =
      ggml_backend_get_default_buffer_type(state_.backend_cpu);
  if (state_.backend_gpu) {
  ...
  state_.sched =
      ggml_backend_sched_new(backends.data(), backend_bufts.data(),
                             backends.size(), QWEN3_FA_MAX_NODES, false, true);
  if (!state_.sched) {
    error_msg_ = "Failed to create backend scheduler";
    return false;
  }

  state_.compute_meta.resize(ggml_tensor_overhead() * QWEN3_FA_MAX_NODES +
                             ggml_graph_overhead());
...
bool ForcedAligner::parse_hparams(struct gguf_context *ctx) {
  ...
  hp.audio_attention_heads =
      get_u32("qwen3-asr.audio.encoder.attention.head_count", 16);
  hp.audio_ffn_dim =
      get_u32("qwen3-asr.audio.encoder.feed_forward_length", 4096);
  ...
  hp.text_rms_norm_eps =
      get_f32("qwen3-asr.attention.layer_norm_rms_epsilon", 1e-6f);
...
bool ForcedAligner::create_tensors(struct gguf_context *ctx) {
  ...
  for (int64_t i = 0; i < n_tensors; ++i) {
    ...
    if (strstr(name, "encoder.conv1.weight")) {
      ...
      ne[1] = 3;
      ne[2] = 1;
      ne[3] = hp.audio_conv_channels;
      n_dims = 4;
    } else if (strstr(name, "encoder.conv2.weight") ||
               strstr(name, "encoder.conv3.weight")) {
      ne[0] = 3;
      ne[1] = 3;
      ne[2] = hp.audio_conv_channels;
      ne[3] = hp.audio_conv_channels;
      n_dims = 4;
    } else if (strstr(name, "encoder.conv1.bias") ||
               strstr(name, "encoder.conv2.bias") ||
      ...
      if (sscanf(name, "audio.encoder.blk.%d.", &layer_idx) == 1 &&
        ...
        if (strstr(name, "attn_q.weight") || strstr(name, "attn_k.weight") ||
          ...
          n_dims = 2;
                   ...
                   strstr(name, "attn_v.bias") ||
                   strstr(name, "attn_out.bias") ||
                   strstr(name, "attn_norm.weight") ||
                   strstr(name, "attn_norm.bias")) {
          ne[0] = hp.audio_d_model;
          n_dims = 1;
          ...
          ne[0] = hp.audio_ffn_dim;
          n_dims = 1;
        } else if (strstr(name, "ffn_down.bias") ||
          ...
          n_dims = 1;
               ...
               strstr(name, "encoder.ln_post.bias")) {
      ne[0] = hp.audio_d_model;
      n_dims = 1;
      ...
      int layer_idx = -1;
      if (sscanf(name, "blk.%d.", &layer_idx) == 1 && layer_idx >= 0 &&
          layer_idx < hp.text_decoder_layers) {

        if (strstr(name, "attn_norm.weight")) {
          ne[0] = hp.text_hidden_size;
          n_dims = 1;
        } else if (strstr(name, "attn_q_norm.weight") ||
                   strstr(name, "attn_k_norm.weight")) {
    ...
    if (strstr(name, "encoder.conv1.weight")) {
      ...
      if (sscanf(name, "audio.encoder.blk.%d.", &layer_idx) == 1 &&
        ...
        if (strstr(name, "attn_q.weight"))
          layer.attn_q_w = tensor;
        else if (strstr(name, "attn_q.bias"))
          layer.attn_q_b = tensor;
        else if (strstr(name, "attn_k.weight"))
          layer.attn_k_w = tensor;
        else if (strstr(name, "attn_k.bias"))
          layer.attn_k_b = tensor;
        else if (strstr(name, "attn_v.weight"))
          layer.attn_v_w = tensor;
        else if (strstr(name, "attn_v.bias"))
          layer.attn_v_b = tensor;
        else if (strstr(name, "attn_out.weight"))
          layer.attn_out_w = tensor;
        else if (strstr(name, "attn_out.bias"))
          layer.attn_out_b = tensor;
        else if (strstr(name, "attn_norm.weight"))
          layer.attn_norm_w = tensor;
        else if (strstr(name, "attn_norm.bias"))
          layer.attn_norm_b = tensor;
        else if (strstr(name, "ffn_up.weight"))
          layer.ffn_up_w = tensor;
        else if (strstr(name, "ffn_up.bias"))
          layer.ffn_up_b = tensor;
        else if (strstr(name, "ffn_down.weight"))
          layer.ffn_down_w = tensor;
        else if (strstr(name, "ffn_down.bias"))
          layer.ffn_down_b = tensor;
        else if (strstr(name, "ffn_norm.weight"))
          layer.ffn_norm_w = tensor;
        else if (strstr(name, "ffn_norm.bias"))
          layer.ffn_norm_b = tensor;
      }
    } else if (strstr(name, "token_embd.weight")) {
...
bool ForcedAligner::load_tensor_data(const std::string &path,
  ...
  if (gpu_dev) {
    model_.buffer = ggml_backend_dev_buffer_from_host_ptr(
        gpu_dev, data_base, total_size, max_tensor_size);
  }
  if (!model_.buffer) {
...
bool ForcedAligner::init_kv_cache(int32_t n_ctx) {
  ...
  for (int il = 0; il < hp.text_decoder_layers; ++il) {
    state_.cache.k_cache[il] =
        ggml_new_tensor_3d(state_.cache.ctx, GGML_TYPE_F16, hp.text_head_dim,
                           hp.text_kv_heads, n_ctx);
    ggml_format_name(state_.cache.k_cache[il], "k_cache_%d", il);

    state_.cache.v_cache[il] =
        ggml_new_tensor_3d(state_.cache.ctx, GGML_TYPE_F16, hp.text_head_dim,
                           hp.text_kv_heads, n_ctx);
  ...
  ggml_backend_t kv_backend =
      state_.backend_gpu ? state_.backend_gpu : state_.backend_cpu;
  state_.cache.buffer =
      ggml_backend_alloc_ctx_tensors(state_.cache.ctx, kv_backend);
...
void ForcedAligner::clear_kv_cache() { state_.cache.n_used = 0; }
...
bool ForcedAligner::encode_audio(const float *mel_data, int n_mel, int n_frames,
  ...
  struct ggml_cgraph *gf =
      ggml_new_graph_custom(ctx0, QWEN3_FA_MAX_NODES, false);

  struct ggml_tensor *mel_batch = ggml_new_tensor_4d(
      ctx0, GGML_TYPE_F32, max_chunk_len, n_mel, 1, n_chunks);
  ggml_set_name(mel_batch, "mel_batch");
  ggml_set_input(mel_batch);

  struct ggml_tensor *cur =
      ggml_conv_2d(ctx0, model_.conv2d1_w, mel_batch, 2, 2, 1, 1, 1, 1);
  if (model_.conv2d1_b) {
    struct ggml_tensor *bias = ggml_reshape_4d(ctx0, model_.conv2d1_b, 1, 1,
                                               hp.audio_conv_channels, 1);
  ...
  if (model_.conv2d2_b) {
    struct ggml_tensor *bias = ggml_reshape_4d(ctx0, model_.conv2d2_b, 1, 1,
                                               hp.audio_conv_channels, 1);
    cur = ggml_add(ctx0, cur, bias);
  }
  cur = ggml_gelu(ctx0, cur);
  ...
  if (model_.conv2d3_b) {
    struct ggml_tensor *bias = ggml_reshape_4d(ctx0, model_.conv2d3_b, 1, 1,
                                               hp.audio_conv_channels, 1);
    cur = ggml_add(ctx0, cur, bias);
  }
  cur = ggml_gelu(ctx0, cur);

  // [out_w, out_h, out_c, n_chunks] -> permute -> conv_out -> [n_state, out_w,
  // n_chunks]
...

  {
    ...
    ggml_backend_tensor_set(mel_t, mel_batch_data.data(), 0,
                            batch_size * sizeof(float));
  }

  if (ggml_backend_sched_graph_compute(state_.sched, gf) !=
      GGML_STATUS_SUCCESS) {
  ...
  ggml_backend_tensor_get(conv_out_t, conv_all.data(), 0,
                          conv_all.size() * sizeof(float));
  ...
  // Windowed attention: window_aftercnn = max_out_w * (n_window_infer /
  // chunk_mel_size)
  ...
  struct ggml_tensor *inp_hidden =
      ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_state, n_ctx);
  ggml_set_name(inp_hidden, "inp_hidden");
  ggml_set_input(inp_hidden);

  struct ggml_tensor *mask_tensor =
      ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_ctx, n_ctx);
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    struct ggml_tensor *Q = ggml_permute(
        ctx0, ggml_reshape_3d(ctx0, Qcur, n_state_head, n_head, n_ctx), 0, 2, 1,
        3);

    struct ggml_tensor *K = ggml_permute(
        ctx0, ggml_reshape_3d(ctx0, Kcur, n_state_head, n_head, n_ctx), 0, 2, 1,
        3);

    struct ggml_tensor *KQ = ggml_mul_mat(ctx0, K, Q);

    struct ggml_tensor *KQ_soft_max =
        ggml_soft_max_ext(ctx0, KQ, mask_tensor, KQscale, 0.0f);
  ...
  ggml_backend_tensor_get(audio_out, output.data(), 0,
                          out_n_ctx * out_n_state * sizeof(float));
...
struct ggml_cgraph *
ForcedAligner::build_decoder_graph(const int32_t *tokens, int32_t n_tokens,
  ...
  if (inp_audio && n_audio > 0 && audio_start_pos >= 0 &&
      audio_start_pos + n_audio <= n_tokens) {
    struct ggml_tensor *embd_before = nullptr;
    struct ggml_tensor *embd_after = nullptr;

    if (audio_start_pos > 0) {
      embd_before =
          ggml_view_2d(ctx0, cur, hidden_size, audio_start_pos, cur->nb[1], 0);
    }

    if (audio_start_pos + n_audio < n_tokens) {
      int after_start = audio_start_pos + n_audio;
      int after_len = n_tokens - after_start;
      embd_after = ggml_view_2d(ctx0, cur, hidden_size, after_len, cur->nb[1],
                                after_start * cur->nb[1]);
    }
  ...
  struct ggml_tensor *causal_mask =
      ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_tokens, n_tokens);
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    Qcur = ggml_rope_ext(ctx0, Qcur, inp_pos, nullptr, head_dim,
                         GGML_ROPE_TYPE_NEOX, 0, rope_theta, 1.0f, 0.0f, 1.0f,
                         0.0f, 0.0f);

    Kcur = ggml_rope_ext(ctx0, Kcur, inp_pos, nullptr, head_dim,
                         GGML_ROPE_TYPE_NEOX, 0, rope_theta, 1.0f, 0.0f, 1.0f,
                         0.0f, 0.0f);

    struct ggml_tensor *Qfa = ggml_permute(ctx0, Qcur, 0, 2, 1, 3);
    struct ggml_tensor *Kfa = ggml_permute(ctx0, Kcur, 0, 2, 1, 3);
    struct ggml_tensor *Vfa =
        ggml_cast(ctx0, ggml_permute(ctx0, Vcur, 0, 2, 1, 3), GGML_TYPE_F16);

    cur = ggml_flash_attn_ext(ctx0, Qfa, Kfa, Vfa, causal_mask, KQscale, 0.0f,
                              0.0f);
...
bool ForcedAligner::forward_decoder(const int32_t *tokens, int32_t n_tokens,
                                    const float *audio_embd, int32_t n_audio,
                                    int32_t audio_start_pos,
                                    std::vector<float> &output) {
  ...
  struct ggml_cgraph *gf = build_decoder_graph(tokens, n_tokens, audio_embd,
                                               n_audio, audio_start_pos);
  if (!gf) {
  ...
  if (audio_embd && n_audio > 0) {
    ...
    if (inp_audio) {
      ggml_backend_tensor_set(inp_audio, audio_embd, 0,
                              n_audio * model_.hparams.text_hidden_size *
                                  sizeof(float));
    }
  }
...
std::vector<int32_t>
ForcedAligner::fix_timestamp_classes(const std::vector<int32_t> &data) {
    ...
    if (!is_normal[i]) {
      ...
      if (anomaly_count <= 2) {
        ...
        if (left_val >= 0 && right_val >= 0) {
          float step =
              static_cast<float>(right_val - left_val) / (anomaly_count + 1);
          for (int k = i; k < j; ++k) {
...
std::vector<float>
ForcedAligner::classes_to_timestamps(const std::vector<int32_t> &classes) {
...
std::vector<int32_t>
ForcedAligner::extract_timestamp_classes(const std::vector<float> &logits,
...
std::vector<int32_t>
ForcedAligner::build_input_tokens(const std::vector<int32_t> &text_tokens,
...
int32_t
ForcedAligner::find_audio_start_pos(const std::vector<int32_t> &tokens) {
  for (size_t i = 0; i < tokens.size(); ++i) {
...
bpe_encode_word(const std::string &word_bpe,
...
static std::string utf8_substr(const std::string &s, size_t char_start,
                               size_t char_count) {
  size_t byte_start = 0;
...
tokenize_korean(const std::string &text,
...
std::vector<int32_t>
ForcedAligner::tokenize_with_timestamps(const std::string &text,
  ...
  for (size_t w = 0; w < raw_words.size(); ++w) {
    ...
    std::vector<std::string> subwords =
        bpe_encode_word(bpe_str, model_.bpe_ranks);
...
alignment_result ForcedAligner::align(const std::string &audio_path,
  ...
  if (sample_rate != QWEN_SAMPLE_RATE) {
    result.error_msg =
        "Audio must be 16kHz, got " + std::to_string(sample_rate) + " Hz";
    return result;
  }
...
alignment_result ForcedAligner::align(const float *samples, int n_samples,
                                      const std::string &text,
                                      const std::string &language) {
  alignment_result result;
  ...
  int32_t n_audio_frames =
      audio_features.size() / model_.hparams.text_hidden_size;
  ...
  std::vector<int32_t> text_tokens =
      tokenize_with_timestamps(text, words, language);

  std::vector<int32_t> input_tokens =
      build_input_tokens(text_tokens, n_audio_pads);
  ...
  if (!forward_decoder(input_tokens.data(), input_tokens.size(),
                       audio_features.data(), n_audio_frames, audio_start_pos,
                       logits)) {
  ...
  for (size_t i = 0; i < words.size(); ++i) {
    ...
    aw.end =
        (end_idx < timestamps.size()) ? timestamps[end_idx] : audio_duration;
