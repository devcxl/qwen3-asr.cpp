...
transcribe_result
Qwen3ASR::transcribe_internal(const float *samples, int n_samples,
                              const transcribe_params &params) {
  transcribe_result result;
  ...
  MelSpectrogram mel;
  {
    ...
    if (!log_mel_spectrogram(samples, n_samples, mel_filters_, mel,
                             params.n_threads)) {
  ...
  std::vector<float> audio_features;
  {
    ...
    if (!encoder_.encode(mel.data.data(), mel.n_mel, mel.n_len,
                         audio_features)) {
  ...
  if (params.print_progress) {
    fprintf(stderr, "Audio features: [%d, %d]\n", n_audio_frames,
            text_hparams.hidden_size);
  }

  std::vector<int32_t> input_tokens =
      build_input_tokens(n_audio_frames, params.language);
  ...
  if (!decode_greedy(input_tokens, audio_features, n_audio_frames, params,
                     output_tokens)) {
...
bool Qwen3ASR::decode_greedy(const std::vector<int32_t> &input_tokens,
  ...
  // Audio pad tokens start after:
  // <|im_start|>system\n<|im_end|>\n<|im_start|>user\n<|audio_start|> That's 8
  // tokens before the first audio_pad
  int32_t audio_start_pos = 9;

  {
    QWEN3_TIMER("decode.initial_forward");
    if (!decoder_.forward_with_audio(input_tokens.data(), input_tokens.size(),
...

    {
      ...
      if (!decoder_.forward(single_token.data(), 1, n_past, logits)) {
        error_msg_ = "Forward pass failed at token " +
                     std::to_string(output_tokens.size()) + ": " +
                     decoder_.get_error();
        return false;
      }
    }
