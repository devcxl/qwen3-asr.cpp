...
bool TextDecoder::load_model(const std::string &model_path) {
  ...
  state_.sched =
      ggml_backend_sched_new(backends.data(), backend_bufts.data(),
                             backends.size(), QWEN3_ASR_MAX_NODES, false, true);
  if (!state_.sched) {
    error_msg_ = "Failed to create backend scheduler";
    return false;
  }

  state_.compute_meta.resize(ggml_tensor_overhead() * QWEN3_ASR_MAX_NODES +
                             ggml_graph_overhead());
  ...
  for (int64_t i = 0; i < n_tensors; ++i) {
    ...
    if (strstr(name, "token_embd.weight")) {
      ...
      if (sscanf(name, "blk.%d.", &layer_idx) == 1 && layer_idx >= 0 &&
          layer_idx < cfg.n_decoder_layers) {
...
bool TextDecoder::load_tensor_data(const std::string &path,
...
bool TextDecoder::init_kv_cache(int32_t n_ctx) {
  ...
  for (int il = 0; il < cfg.n_decoder_layers; ++il) {
    state_.cache.k_cache[il] =
        ggml_new_tensor_3d(state_.cache.ctx, GGML_TYPE_F16, cfg.head_dim,
                           cfg.n_key_value_heads, n_ctx);
...
void TextDecoder::clear_kv_cache() { state_.cache.n_used = 0; }

struct ggml_cgraph *TextDecoder::build_graph(const int32_t *tokens,
  ...
  struct ggml_cgraph *gf =
      ggml_new_graph_custom(ctx0, QWEN3_ASR_MAX_NODES, false);
  ...
  struct ggml_tensor *fa_mask =
      ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_kv, n_tokens);
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    struct ggml_tensor *k_cache_view =
        ggml_view_3d(ctx0, k_cache, head_dim, n_kv_head, n_tokens,
                     k_cache->nb[1], k_cache->nb[2], n_past * k_cache->nb[2]);

    struct ggml_tensor *v_cache_view =
        ggml_view_3d(ctx0, v_cache, head_dim, n_kv_head, n_tokens,
                     v_cache->nb[1], v_cache->nb[2], n_past * v_cache->nb[2]);
    ...
    struct ggml_tensor *K =
        ggml_view_3d(ctx0, k_cache, head_dim, n_kv_head, n_kv, k_cache->nb[1],
                     k_cache->nb[2], 0);

    struct ggml_tensor *V =
        ggml_view_3d(ctx0, v_cache, head_dim, n_kv_head, n_kv, v_cache->nb[1],
                     v_cache->nb[2], 0);

    // flash_attn_ext expects: Q[head_dim, n_tokens, n_head], K[head_dim, n_kv,
    // n_kv_head], V[head_dim, n_kv, n_kv_head]
...
bool TextDecoder::forward(const int32_t *tokens, int32_t n_tokens,
                          int32_t n_past, std::vector<float> &output) {
  return forward_with_audio(tokens, n_tokens, nullptr, 0, -1, n_past, output);
}

bool TextDecoder::forward_with_audio(const int32_t *tokens, int32_t n_tokens,
  ...
  struct ggml_cgraph *gf = build_graph(tokens, n_tokens, n_past, audio_embd,
  ...
  if (fa_mask_t) {
    ...
    ggml_backend_tensor_set(fa_mask_t, mask_data.data(), 0,
  ...
  if (audio_embd && n_audio > 0) {
    ...
    if (inp_audio) {
      ggml_backend_tensor_set(inp_audio, audio_embd, 0,
                              n_audio * model_.config.hidden_size *
                                  sizeof(float));
    }
  }
...
bool TextDecoder::forward_debug(
  ...
  struct ggml_cgraph *gf =
      build_graph(tokens, n_tokens, n_past, nullptr, 0, -1);
  ...
  const char *debug_names[] = {"debug_norm0",     "debug_q0_raw",
                               "debug_q0_normed", "debug_q0_rope",
                               "debug_attn0_out", "debug_kq_scaled",
                               "debug_kq_masked", "debug_kq_softmax"};
...
// GPT-2 byte-level BPE: reverse mapping from Unicode codepoints back to raw
// bytes. See HuggingFace tokenizers bytes_to_unicode() — printable bytes map to
// themselves, non-printable bytes map to codepoints 256+n.
...
std::string TextDecoder::decode_token(int32_t token_id) const {
  ...
  // Byte-level BPE decode: each Unicode codepoint in the token string maps to a
  // byte via the GPT-2 bytes_to_unicode table.
  ...
  while (i < token.size()) {
    ...
    if (cp < cp_to_byte.size() && cp_to_byte[cp] >= 0) {
      ...
      // Codepoint not in BPE table — encode back as UTF-8 (shouldn't happen for
      // valid vocab)
      if (cp < 0x80) {
...
std::string
TextDecoder::decode_tokens(const std::vector<int32_t> &tokens) const {
  std::string result;
