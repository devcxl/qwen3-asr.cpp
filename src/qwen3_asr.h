...
using progress_callback_t =
    std::function<void(int tokens_generated, int max_tokens)>;
...
public:
  ...
  transcribe_result
  transcribe(const std::string &audio_path,
  ...
  transcribe_result
  transcribe(const float *samples, int n_samples,
             const transcribe_params &params = transcribe_params());
  ...
  // Get model config
  const text_decoder_config &get_config() const {
    return decoder_.get_config();
  }
...
private:
  ...
  bool decode_greedy(const std::vector<int32_t> &input_tokens,
                     ...
                     int32_t n_audio_frames, const transcribe_params &params,
