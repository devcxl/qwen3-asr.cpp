...
int main() {
  ...
  printf("\nConv1 weight:\n");
  printf("  Shape: [%lld, %lld, %lld, %lld]\n", (long long)conv1_w->ne[0],
         (long long)conv1_w->ne[1], (long long)conv1_w->ne[2],
         (long long)conv1_w->ne[3]);
  printf("  Type: %d\n", conv1_w->type);
  ...
  ggml_backend_tensor_get(conv1_w, kernel_data.data(), 0,
                          kernel_data.size() * sizeof(float));
  ...
  ggml_backend_tensor_get(conv1_b, bias_data.data(), 0,
