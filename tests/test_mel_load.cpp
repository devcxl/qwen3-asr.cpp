...
static bool load_npy_f32(const char *path, std::vector<float> &data, int &rows,
                         int &cols) {
...
int main() {
  ...
  struct ggml_tensor *mel =
      ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_frames, n_mel);
  ...
  ggml_backend_tensor_get(mel, mel_read.data(), 0,
                          n_mel * n_frames * sizeof(float));
