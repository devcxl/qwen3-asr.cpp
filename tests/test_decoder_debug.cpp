...
static bool load_npy_f32(const std::string &path, std::vector<float> &data,
  ...
  while (pos < shape_str.size()) {
    ...
    int64_t dim = 0;
    while (pos < shape_str.size() && shape_str[pos] >= '0' &&
           shape_str[pos] <= '9') {
  ...
  if (fread(data.data(), sizeof(float), total_elements, f) !=
...
static void compare_arrays(const char *name, const float *a, const float *b,
                           size_t n) {
  float max_diff = 0.0f;
  size_t max_idx = 0;
  float sum_diff = 0.0f;
...
int main() {
  ...
  for (int i = 0; i < 10; ++i)
    printf("%.6f ", ref_logits[i]);
  printf("\n\n");

  std::vector<float> logits;
  std::map<std::string, std::vector<float>> debug_tensors;
  if (!decoder.forward_debug(test_tokens.data(), test_tokens.size(), 0, logits,
                             debug_tensors)) {
  ...
  for (const auto &kv : debug_tensors) {
    printf("%s: size=%zu, first 10 values: ", kv.first.c_str(),
           kv.second.size());
  ...
  if (load_npy_f32("tests/reference/decoder_norm0.npy", ref_norm0,
                   norm0_shape)) {
    printf("Reference norm0 shape: [%lld, %lld, %lld]\n",
           (long long)norm0_shape[0], (long long)norm0_shape[1],
           (long long)norm0_shape[2]);
    printf("Reference norm0[0,0,:10]: ");
    for (int i = 0; i < 10; ++i)
      printf("%.6f ", ref_norm0[i]);
    printf("\n");

    if (debug_tensors.count("debug_norm0")) {
      compare_arrays(
          "Norm0", debug_tensors["debug_norm0"].data(), ref_norm0.data(),
          std::min(debug_tensors["debug_norm0"].size(), ref_norm0.size()));
    }
  }

  if (load_npy_f32("tests/reference/decoder_q0.npy", ref_q0, q0_shape)) {
    printf("\nReference Q0 shape: [%lld, %lld, %lld]\n", (long long)q0_shape[0],
           (long long)q0_shape[1], (long long)q0_shape[2]);
    ...
    if (debug_tensors.count("debug_q0_raw")) {
      compare_arrays(
          "Q0_raw", debug_tensors["debug_q0_raw"].data(), ref_q0.data(),
  ...
  compare_arrays("Logits (pos 0)", logits.data(), ref_logits.data(),
                 cfg.vocab_size);
