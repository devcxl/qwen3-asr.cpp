...
int main() {
  ...
  printf("Mel shape: [%lld, %lld]\n", (long long)mel_shape[0],
         (long long)mel_shape[1]);
  ...
  // The encoder doesn't expose the kernel directly, so let's check via the GGUF
  // file Actually, let's just run the full conv and check the output
