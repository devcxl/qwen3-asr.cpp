...
static void test_find_audio_positions() {
  ...
  auto positions =
      find_audio_positions(input_ids, n_tokens, audio_pad_token_id);
...
static void test_embed_tokens() {
  ...
  embed_tokens(input_ids, n_tokens, token_embd, vocab_size, hidden_size,
               output);
...
static void test_inject_audio_embeddings() {
  ...
  bool success =
      inject_audio_embeddings(token_embeddings, n_tokens, hidden_size,
...
static void test_inject_audio_full() {
  ...
  for (int i = 0; i < vocab_size; ++i) {
    for (int j = 0; j < hidden_size; ++j) {
      token_embd[i * hidden_size + j] =
          static_cast<float>(i % 1000) + static_cast<float>(j) / 10000.0f;
    }
  }
  ...
  for (int i = 0; i < n_audio_frames; ++i) {
    for (int j = 0; j < hidden_size; ++j) {
      audio_features[i * hidden_size + j] =
          static_cast<float>(500 + i) + static_cast<float>(j) / 10000.0f;
    }
  }
  ...
  injection_result result = inject_audio(
      input_ids, n_tokens, audio_features.data(), n_audio_frames, ctx);

  assert(result.success);
  assert(result.seq_len == n_tokens);
  assert(result.hidden_size == hidden_size);
  assert(result.embeddings.size() ==
         static_cast<size_t>(n_tokens * hidden_size));
...
static void test_no_audio() {
  ...
  injection_result result = inject_audio(input_ids, n_tokens, nullptr, 0, ctx);
...
static void test_count_and_find() {
  ...
  int32_t start_pos =
      find_audio_start_position(input_ids, n_tokens, audio_pad_token_id);
  assert(start_pos == 2);
...
static void test_validate_audio_injection() {
  ...
  bool valid = validate_audio_injection(input_ids, n_tokens, 3,
                                        audio_pad_token_id, error_msg);
  assert(valid);

  valid = validate_audio_injection(input_ids, n_tokens, 5, audio_pad_token_id,
                                   error_msg);
  assert(!valid);
