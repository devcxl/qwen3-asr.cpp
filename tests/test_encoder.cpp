...
static float compute_max_abs_diff(const std::vector<float> &a,
...
static float compute_mean_abs_diff(const std::vector<float> &a,
                                   const std::vector<float> &b) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "--model") == 0 && i + 1 < argc) {
      ...
      printf("Options:\n");
      printf("  --model <path>     Path to GGUF model (default: "
             "models/qwen3-asr-0.6b-f16.gguf)\n");
      printf("  --mel <path>       Path to mel spectrogram NPY (default: "
             "tests/reference/mel.npy)\n");
      printf("  --ref <path>       Path to reference output NPY (default: "
             "tests/reference/audio_features.npy)\n");
  ...
  printf("Reference output shape: [%lld, %lld]\n", (long long)ref_shape[0],
         (long long)ref_shape[1]);
