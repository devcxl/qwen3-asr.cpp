...
int main() {
  ...
  if (!load_npy_f32("tests/reference/audio_features.npy", audio_features,
                    audio_shape)) {
    fprintf(stderr, "Failed to load audio features\n");
    return 1;
  }
  printf("Audio features shape: [%lld, %lld]\n", (long long)audio_shape[0],
         (long long)audio_shape[1]);
  ...
  if (!decoder.forward_with_audio(tokens.data(), tokens.size(),
