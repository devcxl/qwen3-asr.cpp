...
bool read_npy_float(const char *filename, std::vector<float> &data,
...
int main() {
  ...
  if (!read_npy_float("/tmp/hf_attn_weights_10.npy", hf_attn_weights,
                      shape_weights)) {
    fprintf(stderr, "Failed to load HF attention weights\n");
    return 1;
  }
  if (!read_npy_float("/tmp/hf_attn_probs_10.npy", hf_attn_probs,
                      shape_probs)) {
  ...
  if (!decoder.forward_debug(tokens.data(), tokens.size(), 0, logits,
                             debug_tensors)) {
  ...
  for (int i = 0; i < 5 && i < seq_len; ++i) {
    for (int j = 0; j < 5 && j < seq_len; ++j) {
      int idx = 0 * n_heads * seq_len * seq_len + 0 * seq_len * seq_len +
                i * seq_len + j;
  ...
  if (debug_tensors.count("debug_kq_softmax")) {
    ...
    for (int i = 0; i < seq_len; ++i) {
      for (int j = 0; j < seq_len; ++j) {
        ...
        int hf_idx = 0 * n_heads * seq_len * seq_len + 0 * seq_len * seq_len +
                     i * seq_len + j;
