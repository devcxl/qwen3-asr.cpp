...
int main() {
  ...
  float kernel_pt[OC * IC * KH * KW] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
  ...
  // (0,-1)  (0,0)  (0,1)    ->  0  1  2
  // (1,-1)  (1,0)  (1,1)    ->  0  5  6
  // Convolution: 0*1 + 0*2 + 0*3 + 0*4 + 1*5 + 2*6 + 0*7 + 5*8 + 6*9 = 5 + 12 +
  // 40 + 54 = 111 Wait, that's wrong. Let me recalculate... Kernel is [kh, kw],
  // input region is [ih, iw] For output (oh=0, ow=0) with stride=1, padding=1:
  ...
  struct ggml_tensor *input =
      ggml_new_tensor_4d(ctx, GGML_TYPE_F32, IW, IH, IC, N);
  ggml_set_name(input, "input");
  ggml_set_input(input);

  // Kernel: GGML expects [KW, KH, IC, OC]
  struct ggml_tensor *kernel =
      ggml_new_tensor_4d(ctx, GGML_TYPE_F32, KW, KH, IC, OC);
  ggml_set_name(kernel, "kernel");
  ggml_set_input(kernel);

  // Conv2d
  struct ggml_tensor *output =
      ggml_conv_2d(ctx, kernel, input, stride, stride, padding, padding, 1, 1);
  ...
  // Set input data
  // GGML input is [IW, IH, IC, N], element [w, h, c, n] is at offset w + h*IW +
  // c*IW*IH + n*IW*IH*IC PyTorch input is [N, IC, IH, IW], element [n, c, h, w]
  // is at offset w + h*IW + c*IH*IW + n*IC*IH*IW So GGML [w, h, 0, 0] = PyTorch
  // [0, 0, h, w] = input_data[h * IW + w] The memory layout is the same!
